...
function startGame() { resetGame(); started.value = true; startLoop() }
...
function startLoop(){
  ...
  const frame = (ts:number)=>{
    ...
    if (started.value && !paused.value && !gameOver.value) {
      if (ts - lastTick >= getTickMs()) { doTick(); lastTick = ts }
...
function drawSnake(t=0){ if(!ctx||snake.value.length===0) return; const centers = snake.value.map((p,i)=>{ const pp=prevSnake[i]??p; const rx=pp.x+(p.x-pp.x)*t, ry=pp.y+(p.y-pp.y)*t; return {x:rx*CELL+CELL/2,y:ry*CELL+CELL/2} }); const tail=[...centers].reverse(); ctx.save(); ctx.lineJoin='round'; ctx.lineCap='round'; for(let i=1;i<tail.length;i++){ const p1=tail[i-1],p2=tail[i]; const scale=0.55+0.45*(i/(tail.length-1)); ctx.strokeStyle='rgba(0,0,0,0.25)'; ctx.lineWidth=CELL*scale*0.95; ctx.beginPath(); ctx.moveTo(p1.x+1.2,p1.y+1.2); ctx.lineTo(p2.x+1.2,p2.y+1.2); ctx.stroke() } for(let i=1;i<tail.length;i++){ const p1=tail[i-1],p2=tail[i]; const scale=0.55+0.45*(i/(tail.length-1)); const grd=ctx.createLinearGradient(p1.x,p1.y,p2.x,p2.y); grd.addColorStop(0,'#14b8a6'); grd.addColorStop(1,'#22d3ee'); ctx.strokeStyle=grd; ctx.lineWidth=CELL*scale; ctx.beginPath(); ctx.moveTo(p1.x,p1.y); ctx.lineTo(p2.x,p2.y); ctx.stroke() } const head=centers[0], second=centers[1]??head; const dx=head.x-second.x, dy=head.y-second.y; const len=Math.hypot(dx,dy)||1; const nx=dx/len, ny=dy/len; const eyeOff=CELL*0.28, px=-ny, py=nx; const e1={x:head.x+px*eyeOff,y:head.y+py*eyeOff}, e2={x:head.x-px*eyeOff,y:head.y-py*eyeOff}; ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(e1.x,e1.y,CELL*0.14,0,Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(e2.x,e2.y,CELL*0.14,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#0b1220'; ctx.beginPath(); ctx.arc(e1.x+nx*CELL*0.06,e1.y+ny*CELL*0.06,CELL*0.07,0,Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(e2.x+nx*CELL*0.06,e2.y+ny*CELL*0.06,CELL*0.07,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#f43f5e'; ctx.beginPath(); const tip={x:head.x+nx*CELL*0.7,y:head.y+ny*CELL*0.7}, tl={x:head.x+nx*CELL*0.35+px*CELL*0.06,y:head.y+ny*CELL*0.35+py*CELL*0.06}, tr={x:head.x+nx*CELL*0.35-px*CELL*0.06,y:head.y+ny*CELL*0.35-py*CELL*0.06}; ctx.moveTo(tl.x,tl.y); ctx.lineTo(tip.x,tip.y); ctx.lineTo(tr.x,tr.y); ctx.closePath(); ctx.fill(); ctx.restore() }

function draw(t=0){ if(!ctx) return; drawBg(); drawFoods(); drawPenalties(); drawSnake(t); if (paused.value || gameOver.value){ ctx.fillStyle='rgba(0,0,0,0.35)'; ctx.fillRect(0,0,W,H); ctx.fillStyle='#fff'; ctx.font='bold 28px system-ui, -apple-system, Segoe UI, Roboto, sans-serif'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(gameOver.value?'游戏结束':'暂停中', W/2, H/2-20); ctx.font='16px system-ui, -apple-system, Segoe UI, Roboto, sans-serif'; ctx.fillText('R 重开，空格 暂停/继续', W/2, H/2+14) } }

onMounted(()=>{ loadScores(); const c=canvasRef.value; if(!c) return; const g=c.getContext('2d'); if(!g) return; ctx=g; resetGame(); draw(0); window.addEventListener('keydown', handleKey); window.addEventListener('keydown', handleControlKeys); (document.body as HTMLBodyElement).dataset.prevOverflow = document.body.style.overflow; document.body.style.overflow='hidden' })
